...
// Client-only: the session token lives in memory (see AuthContext), so this can't be fetched server-side.
export function ConversationThreadView({ conversationId }: ConversationThreadViewProps) {
  ...
  // Same "derived loading" pattern as FavoritesContext: avoids calling
  // setState synchronously at the top of the effect just to flip a loading
  // flag before the fetch even starts.
  ...
  useEffect(() => {
    ...
    fetch(`/api/conversations/${conversationId}/messages`, {
      ...
      .then((response) => {
        // 404 covers both "conversation doesn't exist" and "not a member" —
        // see the messaging API's anti-enumeration rule (never 403).
      ...
      .catch((reason: unknown) => {
        ...
        setResult({
          status: reason instanceof Error && reason.message === 'not-found' ? 'not-found' : 'error',
        });
      })
      .finally(() => {
        if (!cancelled) {
